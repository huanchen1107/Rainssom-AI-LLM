...
def load_rag_pipeline():
    ...
    contextualize_q_chain = contextualize_q_prompt | Ollama(model="llama3-70b-final", temperature=0.1) | StrOutputParser() # <--- 第一次修改
    ...
    qa_chain = qa_prompt | Ollama(model="llama3-70b-final", temperature=0.1) | StrOutputParser() # <--- 第二次修改
...
# ------------------------------------------------------------------
# 2. Streamlit 應用程式主體 (這部分完全不用動)
# ------------------------------------------------------------------

# 設定頁面標題
st.title("🚀 Rainssom 醫美智能助理 (70B)") # 我在標題加了 (70B) 讓您好辨認
